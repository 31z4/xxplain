...
def run_explain_analyze_json(cur, query: str, analyze=False) -> dict:
    ...
    with conn.cursor() as cur:
        ...
        for q in queries:
            ...
            try:
                ...
                plan_analyze_json = run_explain_analyze_json(cur, q, analyze=True)

                yield {
                    "query": q,
                    "plan": json.dumps(plan_json[0]),
                    "plan_analyze": json.dumps(plan_analyze_json[0]),
                }
...
if __name__ == "__main__":
    ...
    with open(csv_filename, mode="w", newline="") as file:
        writer = csv.DictWriter(
            file, fieldnames=["query", "plan", "plan_analyze"], delimiter="\t", quoting=csv.QUOTE_NONE, quotechar=None
        )
        writer.writeheader()
