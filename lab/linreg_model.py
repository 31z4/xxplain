...
def fit_linear_regression(X_train: pd.DataFrame, y_train: pd.Series):
    """
    Обучает линейную регрессию. Если sklearn недоступен, используется решение в смысле МНК через pinv.
    """
...
def main():
    ...
    out_preds_path = "lab/test_with_pred.csv"
    out_coefs_path = "lab/linreg_coefs.csv"
    ...
    test_out.to_csv(out_preds_path, index=False)
    print(f"Предсказания сохранены в: {out_preds_path}")

    # Сохраняем коэффициенты (если sklearn-бэкенд)
    try:
        coefs = pd.DataFrame({
            "feature": X_train.columns,
            "coef": model.coef_.astype(float),
        }).sort_values("coef", key=lambda s: s.abs(), ascending=False)
        coefs.to_csv(out_coefs_path, index=False)
        print(f"Коэффициенты модели сохранены в: {out_coefs_path}")
    except Exception as e:
        print(f"Не удалось сохранить коэффициенты модели: {e}")
